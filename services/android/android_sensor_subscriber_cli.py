...
"""Print Android-origin navigation sensor samples received over ZeroMQ."""
...
def main() -> int:
    ...
    print(f"[*] Subscribed to {IMU_STATE_TOPIC} and {MAGNETIC_FIELD_STATE_TOPIC}")
    ...
    try:
        while True:
            ...
            if topic == IMU_STATE_TOPIC:
                ...
                print(f"IMU accel=({a.x:.3f}, {a.y:.3f}, {a.z:.3f}) gyro=({g.x:.3f}, {g.y:.3f}, {g.z:.3f})", flush=True)
            elif topic == MAGNETIC_FIELD_STATE_TOPIC:
                message = decode_magnetic_field_state(payload)
                if message.source != ANDROID_SENSOR_SOURCE:
                    continue
                m = message.magnetic_field_ut
                print(f"MAG field=({m.x:.2f}, {m.y:.2f}, {m.z:.2f}) uT", flush=True)
