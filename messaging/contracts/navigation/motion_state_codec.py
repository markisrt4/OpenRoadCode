...
def encode_motion_state(*, timestamp: Mapping[str, int], source: str, heading_rad=None, ground_speed_m_s=None, vertical_speed_m_s=None, turn_rate_rad_s=None, is_cached=False) -> dict[str, Any]:
...
def decode_motion_state(payload: Mapping[str, Any]) -> MotionStateMessage:
    ...
    return MotionStateMessage(
        ...
        data=MotionStateData(**{name: data[name] for name in MotionStateData.__dataclass_fields__}),
    )
