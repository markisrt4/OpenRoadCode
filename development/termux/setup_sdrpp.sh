...
apt-get update
apt-get install -y \
  build-essential cmake git binutils python3 \
  libfftw3-dev libglfw3-dev libvolk-dev libzstd-dev libusb-1.0-0-dev \
  librtlsdr-dev libsoapysdr-dev librtaudio-dev libhackrf-dev
...
for instance, module in entries:
...
for module in "$REMOTE_CONTROL_MODULE" "$TELEMETRY_MODULE"; do
  ...
  for symbol in _INFO_ _INIT_ _CREATE_INSTANCE_ _DELETE_INSTANCE_ _END_; do
    if ! nm -D "$module" 2>/dev/null | grep -Eq "[[:space:]]${symbol}$"; then
