...
In the Android bridge, add a remote OpenRoadCode device using `http://HOST:8769` and choose **PAIR IN BROWSER**. The service manager returns a short-lived approval URL containing an opaque, single-session approval capability. The browser shows the requesting Android client and an **Approve device** button; no administrator token or PIN needs to be copied or entered. After approval, the Android client receives and stores its own client credential. The target can be a Linux VM. Verify status refresh and switching between Termux and the selected remote before issuing service actions.
