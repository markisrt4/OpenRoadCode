...
In the Android bridge, add a remote OpenRoadCode device using `http://HOST:8769` and choose **PAIR IN BROWSER**. Remote browser approval uses a short-lived six-digit pairing PIN rather than exposing the permanent administrator bearer token. On a systemd host, read the current PIN locally with `sudo journalctl -u openroadcode-service-manager -n 20 --no-pager`, enter it in the approval page, then return to the app. The Android client receives and stores its own client credential after approval. The target can be a Linux VM. Verify status refresh and switching between Termux and the selected remote before issuing service actions.
