...
ADDRESS="${BLUETOOTH_SPP_ADDRESS:-${ELM327_ADDRESS:-}}"
CHANNEL="${BLUETOOTH_SPP_CHANNEL:-${ELM327_CHANNEL:-}}"
RFCOMM_ID="${BLUETOOTH_RFCOMM_ID:-${ELM327_RFCOMM_ID:-0}}"

usage() {
  cat <<'EOF'
Usage: setup_bluetooth_spp.sh [--address MAC] [options]

Pair, trust, and bind a Bluetooth Classic Serial Port Profile device.

Options:
  --address MAC       Bluetooth device address; interactively selected when omitted
  --channel NUMBER    RFCOMM channel; auto-detected when omitted
  --rfcomm-id NUMBER  RFCOMM device number (default: 0)
  -h, --help          Show this help

Environment alternatives: BLUETOOTH_SPP_ADDRESS, BLUETOOTH_SPP_CHANNEL,
BLUETOOTH_RFCOMM_ID.
EOF
}
...
if [[ -z "$ADDRESS" ]]; then
  if [[ ! -t 0 ]]; then
    echo "[!] --address is required in a noninteractive session." >&2
    exit 1
  fi

  echo "[*] Scanning for Bluetooth devices for up to 15 seconds..."
  bluetoothctl --timeout 15 scan on >/dev/null 2>&1 || true
  mapfile -t discovered_devices < <(
    bluetoothctl devices \
      | sed -n -E 's/^Device ([[:xdigit:]:]{17}) (.+)$/\1|\2/p'
  )

  if (( ${#discovered_devices[@]} > 0 )); then
    echo "[*] Discovered or previously known devices:"
    for index in "${!discovered_devices[@]}"; do
      entry="${discovered_devices[$index]}"
      echo "    $((index + 1))) ${entry#*|} (${entry%%|*})"
    done
    echo "    0) Enter an address manually"
    read -r -p "Select a device [0]: " selection
    ...
    if (( selection_number <= ${#discovered_devices[@]} && selection_number > 0 )); then
      ADDRESS="${discovered_devices[$((selection_number - 1))]%%|*}"
    fi
  fi

  if [[ -z "$ADDRESS" ]]; then
    read -r -p "Bluetooth device address: " ADDRESS
  fi
fi
...
echo "[+] Bluetooth SPP device configured: $device_path"
ls -l "$device_path"
