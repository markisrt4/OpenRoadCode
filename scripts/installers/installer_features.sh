...
is_known_feature() {
  ...
  while read -r feature; do [[ "$requested" == "$feature" ]] && return 0; done < <(get_known_features)
  return 1
}
...
get_feature_packages() {
  local feature="$1"
  case "$feature" in
    base) echo "git curl wget ca-certificates python3 python3-venv python3-pip" ;;
    desktop-ui) echo "python3-tk dbus-x11 xauth xterm x11-apps wmctrl openbox xfce4 xfce4-goodies" ;;
    browser) echo "" ;;
    vnc) echo "tigervnc-standalone-server tigervnc-common" ;;
    input) echo "" ;;
    gps) echo "gpsd gpsd-clients python3-gps" ;;
    rtl-sdr) echo "rtl-sdr soapysdr-tools soapysdr-module-rtlsdr" ;;
    streamlit) echo "" ;;
    adsb) echo "readsb" ;;
    bluetooth) echo "bluez libbluetooth-dev python3-bluez" ;;
    automotive) echo "python3-serial libserial-dev can-utils" ;;
    imu|environmental|raspberry-pi) echo "i2c-tools" ;;
    spotify)
...
get_feature_python_packages() {
  ...
  case "$feature" in
    ...
    desktop-ui) printf '%s\n' Pillow ;;
    browser|vnc|adsb|spotify|sdrpp) echo "" ;;
    input) printf '%s\n' evdev ;;
    gps) printf '%s\n' gpsd-py3 geocoder ;;
    rtl-sdr) echo "" ;;
    ...
    bluetooth) printf '%s\n' bleak ;;
    automotive) printf '%s\n' pyserial ;;
    imu|environmental) echo "" ;;
    raspberry-pi)
      printf '%s\n' raspberry-pi-gpio-backend adafruit-blinka adafruit-circuitpython-seesaw
      ;;
    *) echo "" ;;
  esac
}
