...
class BleakGattTransport(BleGattTransportIf):
    ...
    def __init__(
        ...
        *,
        ...
        self._excluded_service_uuids = {
            value.lower() for value in excluded_service_uuids
        }
        self._excluded_name_fragments = tuple(
            value.lower() for value in excluded_name_fragments
        )
    ...
    async def _connect_unlocked(self) -> None:
        ...
        if self._address is not None:
            client = await self._connect_address()
        else:
            client = await self._discover_target()
        self._client = client
        self._set_state(BleGattStatus.CONNECTED)
    ...
    async def _connect_address(self) -> BleakClient:
        ...
        if device is None:
            raise RuntimeError(
                f"BLE device {self._address} was not visible during a "
                f"{self._scan_timeout_seconds:g}-second scan"
            )
        ...
        if not self._has_target_characteristic(client):
            ...
            raise RuntimeError(
                f"Device {self._address} does not expose characteristic "
    ...
    async def _discover_target(self) -> BleakClient:
        ...
        candidates = [
            ...
            if not self._is_excluded(device, advertisement)
        ]
        candidates.sort(
            key=lambda item: getattr(item[1], "rssi", -999),
            reverse=True,
        )
        ...
        raise RuntimeError(
            f"No BLE device exposed characteristic "
    ...
    def _has_target_characteristic(self, client: BleakClient) -> bool:
        return any(
            characteristic.uuid.lower() == self._characteristic_uuid
    ...
    def _is_excluded(self, device: BLEDevice, advertisement: Any) -> bool:
        ...
        services = {
            value.lower()
            for value in getattr(advertisement, "service_uuids", ())
        }
        return (
            any(fragment in name for fragment in self._excluded_name_fragments)
            or bool(services & self._excluded_service_uuids)
        )
    ...
    def _on_disconnected(self, client: BleakClient) -> None:
        ...
        if self._intentional_disconnect:
            ...
            self._set_state(
                BleGattStatus.RECONNECTING,
                error="Bluetooth connection was lost",
            )

    def _set_state(
        self,
        status: BleGattStatus,
        *,
        error: str | None = None,
    ) -> None:
        state = BleGattState(
