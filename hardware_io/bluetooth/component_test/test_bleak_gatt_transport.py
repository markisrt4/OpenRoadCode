...
_module = importlib.import_module(
    "hardware_io.bluetooth.bleak_gatt_transport"
)
...
class BleakGattTransportTest(unittest.TestCase):
    ...
    def test_discovers_connects_and_writes_one_characteristic(self) -> None:
        ...
        self.assertEqual(
            BleGattStatus.CONNECTED,
            transport.current_state().status,
        )
        self.assertEqual(
            [b"lighting-command"],
            _Client.instances[0].writes,
        )

    def test_disconnect_only_closes_transport_owned_client(self) -> None:
        unrelated_client = _Client(_Device("11:22:33:44:55:66"), disconnected_callback=lambda _client: None)
        ...
        self.assertEqual(
            BleGattStatus.DISCONNECTED,
            transport.current_state().status,
        )

    def test_ble_disconnect_does_not_close_elm327_rfcomm_serial(self) -> None:
        ...
        elm327 = Elm327Device()
        elm327._serial = SerialConnection()
        transport = _module.BleakGattTransport(
            characteristic_uuid=_Characteristic.uuid,
            command_delay_seconds=0,
        )
        ...
        self.assertTrue(elm327.is_connected)
        elm327.disconnect()
