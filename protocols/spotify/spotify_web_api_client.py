...
from protocols.spotify.spotify_auth import SpotifyAuth
...
class SpotifyWebApiError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(f"Spotify HTTP {status_code}: {response_body}")
        ...
        try:
            with urllib.request.urlopen(
                ...
                if not isinstance(parsed, dict):
                    raise RuntimeError("Spotify response was not a JSON object")
                return parsed
        except urllib.error.HTTPError as exc:
            self._raise_http_error(exc)
        except urllib.error.URLError as exc:
            raise RuntimeError(f"Unable to reach Spotify Web API: {exc}") from exc
        return None
