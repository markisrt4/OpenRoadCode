(()=>{
  ...
  function filterActivity(activity,a){if(!activity)return activity;if(zeroizeUntil)return{kick:0,bass:0,snare:0,cymbal:0,tomHigh:0,tomMid:0,tomLow:0};if(!zeroBaselineDb)return activity;const snareGate=Math.max(rangeGate(160,300,a),rangeGate(1500,5200,a));return{kick:(activity.kick||0)*rangeGate(45,105,a),bass:(activity.bass||0)*rangeGate(55,260,a),snare:(activity.snare||0)*snareGate,cymbal:(activity.cymbal||0)*rangeGate(6000,15000,a),tomHigh:(activity.tomHigh||0)*rangeGate(145,260,a),tomMid:(activity.tomMid||0)*rangeGate(105,195,a),tomLow:(activity.tomLow||0)*rangeGate(70,135,a)}}
