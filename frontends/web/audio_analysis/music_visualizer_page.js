(()=>{
  ...
  function upgradeKit(){
    const kit=document.querySelector('#music-drum-kit .mv-kit');
    if(!kit)return false;
    if(document.getElementById('drum-tom-mid'))return true;
    kit.innerHTML=`
      <div id="drum-cymbal-left" class="mv-cymbal mv-cymbal-left"></div><span class="mv-kit-name mv-name-hihat">HI-HAT</span>
      <div id="drum-cymbal-right" class="mv-cymbal mv-cymbal-right"></div><span class="mv-kit-name mv-name-crash">CRASH</span>
      <div id="drum-tom-high" class="mv-drum mv-tom mv-tom-high"><span class="mv-drum-label">HIGH TOM</span></div>
      <div id="drum-tom-mid" class="mv-drum mv-tom mv-tom-mid"><span class="mv-drum-label">MID TOM</span></div>
      <div id="drum-tom-low" class="mv-drum mv-tom mv-tom-low"><span class="mv-drum-label">LOW TOM</span></div>
      <div id="drum-snare" class="mv-drum mv-snare"><span class="mv-drum-label">SNARE</span></div>
      <div id="drum-kick-left" class="mv-drum mv-kick mv-kick-left"><span class="mv-drum-label">LEFT KICK</span></div>
      <div id="drum-kick-right" class="mv-drum mv-kick mv-kick-right"><span class="mv-drum-label">RIGHT KICK</span></div>`;
    return true;
  }

  function upgradeMeters(){
    const grid=document.querySelector('#mv-activity .mv-meter-grid');
    if(!grid||document.getElementById('mv-seg-tomMid'))return !!grid;
    const high=document.getElementById('mv-seg-tomHigh')?.closest('.mv-meter');
    if(!high)return false;
    const meter=document.createElement('div');
    meter.className='mv-meter mv-tom-mid-color';
    meter.innerHTML=`<div class="mv-meter-name">MID TOM</div><div id="mv-seg-tomMid" class="mv-segments">${'<i class="mv-segment"></i>'.repeat(12)}</div>`;
    high.after(meter);
    grid.classList.add('mv-meter-grid-six');
    return true;
  }

  let uiAttempts=0;
  const upgradeUi=()=>{
    const link=document.querySelector('link[data-mv]');
    if(link&&!link.href.includes('v=5'))link.href='/web-assets/audio-analysis/music_visualizer.css?v=5';
    if(upgradeKit()&&upgradeMeters())return;
    if(uiAttempts++<40)setTimeout(upgradeUi,50);
  };
  ...
  let midTomPeak=1e-6,previousMidTom=0;
  let previousKickForOnset=0,lastKickOnsetAt=-Infinity,nextKickLeft=true,kickLeftEnvelope=0,kickRightEnvelope=0;
  ...
  const drumVisual={snare:{previous:0,envelope:0,decay:.66,threshold:.085,boost:1},tomHigh:{previous:0,envelope:0,decay:.72,threshold:.09,boost:.95},tomMid:{previous:0,envelope:0,decay:.71,threshold:.085,boost:.98},tomLow:{previous:0,envelope:0,decay:.73,threshold:.09,boost:.98},cymbal:{previous:0,envelope:0,decay:.88,threshold:.075,boost:.72}};
  ...
  const deriveMidTom=activity=>{
    if(!analyzer.audioContext||!analyzer.frequencyData)return 0;
    const raw=analyzer.raw(105,195);
    midTomPeak=Math.max(raw,midTomPeak*.994,1e-6);
    const normalized=Math.min(1,raw/midTomPeak),rise=Math.max(0,normalized-previousMidTom);
    previousMidTom=normalized;
    return Math.max(0,Math.min(1,normalized*.11+rise*2.3)-(activity?.bass||0)*.18-(activity?.kick||0)*.10);
  };

  const renderMidMeter=value=>{
    const seg=document.getElementById('mv-seg-tomMid');if(!seg)return;
    const on=Math.round(Math.max(0,Math.min(1,value))*12),color='#38d6b4';
    [...seg.children].forEach((x,i)=>{const active=i<on;x.style.background=active?color:'#18222e';x.style.boxShadow=active&&i>=on-2?`0 0 9px ${color}`:'none';x.style.opacity=active?1:.55});
  };

  const updateKickOnsets=kick=>{
    const now=performance.now(),rise=Math.max(0,kick-previousKickForOnset);
    previousKickForOnset=kick;
    if(rise>.095&&kick>.22&&now-lastKickOnsetAt>62){
      const impulse=Math.min(1,.48+rise*3.5+kick*.25);
      if(nextKickLeft)kickLeftEnvelope=Math.max(kickLeftEnvelope,impulse);else kickRightEnvelope=Math.max(kickRightEnvelope,impulse);
      nextKickLeft=!nextKickLeft;lastKickOnsetAt=now;
    }
    kickLeftEnvelope*=.70;kickRightEnvelope*=.70;
    return {left:Math.max(kick*.18,kickLeftEnvelope),right:Math.max(kick*.18,kickRightEnvelope)};
  };

  const emphasizeDrums=activity=>{
    if(!activity)return;
    const kicks=updateKickOnsets(activity.kick||0),snare=updateEnvelope('snare',activity.snare||0),tomHigh=updateEnvelope('tomHigh',activity.tomHigh||0),tomMid=updateEnvelope('tomMid',activity.tomMid||0),tomLow=updateEnvelope('tomLow',activity.tomLow||0),cymbal=updateEnvelope('cymbal',activity.cymbal||0);
    setPunch('drum-kick-left',kicks.left,'rgba(255,73,57,.62)','translateX(-50%)');
    setPunch('drum-kick-right',kicks.right,'rgba(255,73,57,.62)','translateX(-50%)');
    setPunch('drum-snare',snare,'rgba(255,198,46,.62)','translateX(-50%)');
    setPunch('drum-tom-high',tomHigh,'rgba(168,76,255,.62)');
    setPunch('drum-tom-mid',tomMid,'rgba(56,214,180,.62)');
  ...
  const render=state=>{
    if(state.activity)state.activity.tomMid=deriveMidTom(state.activity);
    if(root.SpectrumDisplay?.filterActivity&&state.activity)state.activity=root.SpectrumDisplay.filterActivity(state.activity,analyzer);
    ...
    renderMidMeter(state.activity?.tomMid||0);
