(()=>{
  ...
  function classify(a,activity){
    ...
    const quiet={...activity,kick:0,snare:0,tomLow:0,tomMid:0,tomHigh:0,hit:null};
    ...
    const scores=[['kick',kick],['snare',snare],['tomLow',tomLow],['tomMid',tomMid],['tomHigh',tomHigh]].sort((x,y)=>y[1]-x[1]),best=scores[0][1],second=scores[1][1];if(best<.20)return quiet;
    lastEventAt=now;const out={...quiet};out[scores[0][0]]=clamp(.45+best*.55);out.hit={id:++eventId,type:scores[0][0],strength:clamp(best)};if(second>best*.84)out[scores[1][0]]=clamp(second*.38);return out;
  }
