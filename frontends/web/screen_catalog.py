...
def create_web_screens() -> dict[str, WebScreen]:
    ...
    return {
        ...
        "weather_overview": WebScreen("Weather", "Frontend provider shell", '''<div class="hero-value">72°<small>F</small></div><div class="card"><b>Current Conditions</b><p>Partly cloudy</p></div><div class="stat-grid"><div class="stat"><b>8 mph</b><small>WIND</small></div><div class="stat"><b>29.96</b><small>PRESSURE</small></div><div class="stat"><b>48%</b><small>HUMIDITY</small></div></div><div class="notice">Demo data for mobile UI development.</div>'''),
        ...
        "offroad_dashboard": WebScreen(
            ...
            <div class="card"><b>Phone sensors</b><p id="sensor-status" class="sensor-status">Tap START SENSORS to request browser access.</p><div id="location" class="sensor-status">GPS: --</div><button id="start-sensors" class="primary wide">START SENSORS</button></div>
            ...
            (() => {
              ...
              const value=(id,v,digits=1,suffix='°') => { document.getElementById(id).textContent=Number.isFinite(v)?`${v.toFixed(digits)}${suffix}`:'--'; };
              document.getElementById('start-sensors').addEventListener('click', async () => {
                const messages=[];
                try {
                  const granted=await orientation.requestPermission();
                  if (granted) {
                    orientation.start((sample)=>{value('pitch',sample.pitch);value('roll',sample.roll);value('heading',sample.heading,0);},(error)=>{status.textContent=`Orientation: ${error.message}`;});
                ...
                geolocation.start((sample)=>{
                  const mph=Number.isFinite(sample.speed)?sample.speed*2.236936:null;
                  value('speed',mph,1,' mph');
                  document.getElementById('location').textContent=`GPS: ${sample.latitude.toFixed(6)}, ${sample.longitude.toFixed(6)} ±${Math.round(sample.accuracy)} m`;
                  if (Number.isFinite(sample.heading)) value('heading',sample.heading,0);
