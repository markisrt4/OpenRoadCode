...
<div id="music-song-card" class="card" style="border-color:#30475d;background:linear-gradient(145deg,#121d28,#111820)"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px"><div><b style="font-size:18px">NOW HEARING</b><div id="music-song-provider" style="color:#8fa0ad;font-size:12px;margin-top:3px">Checking song recognition…</div></div><span id="music-song-state" style="font-size:11px;font-weight:900;letter-spacing:.05em;padding:5px 9px;border:1px solid #42566a;border-radius:999px;color:#aebac4">CHECKING</span></div><div style="display:flex;align-items:center;gap:16px;margin:18px 0"><div style="width:72px;height:72px;flex:0 0 72px;border-radius:14px;background:radial-gradient(circle at 35% 30%,#31557a,#172431 70%);display:grid;place-items:center;font-size:32px;color:#a9c9e8">♫</div><div style="min-width:0"><div id="music-song-title" style="font-size:22px;font-weight:900">No song identified</div><div id="music-song-artist" style="color:#c0cad4;margin-top:4px">Waiting for recognition</div><div id="music-song-album" style="color:#81909e;font-size:13px;margin-top:4px">Album metadata will appear here</div></div></div><button id="music-song-identify" class="wide" type="button" disabled>IDENTIFY SONG</button><div id="music-song-status" style="color:#8fa0ad;font-size:12px;margin-top:9px">Loading recognition controller state…</div></div>
...
<script>(async()=>{const state=document.getElementById('music-song-state'),provider=document.getElementById('music-song-provider'),status=document.getElementById('music-song-status'),button=document.getElementById('music-song-identify');try{const r=await fetch('/api/song-recognition/config'),c=await r.json();if(c.configured){state.textContent='READY';state.style.color='#62dda2';provider.textContent=(c.provider||'song recognition').toUpperCase();status.textContent='Recognition provider ready.';button.disabled=false}else{state.textContent='UNCONFIGURED';state.style.color='#ffc75b';provider.textContent='Song recognition controller';status.textContent='No recognition provider configured.';button.disabled=true}}catch(e){state.textContent='OFFLINE';status.textContent=e.message;button.disabled=true}})();</script>
<script src="/web-assets/audio-analysis/microphone_analyzer.js?v=7"></script>
<script src="/web-assets/audio-analysis/spectrum_controls.js?v=5"></script>
<script src="/web-assets/audio-analysis/kick_mode.js?v=2"></script>
<script src="/web-assets/audio-analysis/webgl_music_visualizer.js?v=3"></script>
<script src="/web-assets/audio-analysis/music_visualizer_page.js?v=17"></script>
<script src="/web-assets/audio-analysis/linux_audio_source.js?v=2"></script>
'''
