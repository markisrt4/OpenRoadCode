...
class SpotifyScreen(TkScreen, MediaUiIf):
    ...
    def __init__(
        ...
        back_action: Callable[[], None], image_cache: ArtworkProviderIf,
    ...
    def _browse_panel(self, parent: tk.Misc) -> SpotifyBrowsePanel:
        ...
        return SpotifyBrowsePanel(
            parent, service=self._service, local_player=self._local_player,
            dispatch_ui=self._dispatch_ui, show_now_playing=self._show_now_playing,
            image_cache=self._image_cache, theme=self._theme,
        )
    ...
    def _show_now_playing(self) -> None:
        ...
        panel = _ThreadSafeSpotifyPlaybackPanel(
            parent=content, theme=self._theme, image_cache=self._image_cache,
            lyrics_client=self._lyrics_client, music_video_controller=self._music_video_controller,
        ...
        self._video_overlay = SpotifyVideoOverlay(
            content, controller=self._music_video_controller, presentation=self._music_video_presentation,
            on_returned=self._request_spotify_refresh, set_status=self._host.set_screen_status,
        )
    ...
    def _poll_state(self, panel: SpotifyPlaybackPanel, generation: int) -> None:
        ...
        self._refresh_job = self._host.schedule_ui_callback(
            self._theme["layout"]["refresh_interval_ms"], lambda: self._start_refresh(panel, generation),
        )
