...
class AppRuntimeManagerTest(unittest.TestCase):
    ...
    def test_generic_window_preload_launches_then_hides(self) -> None:
        ...
        self.assertEqual(1, launcher.hides)
        self.assertTrue(manager.is_running("weather"))
        self.assertFalse(manager.is_visible("weather"))
    ...
    def test_preloaded_window_show_restores_without_second_launch(self) -> None:
        ...
        manager.show("weather")

        self.assertEqual(1, launcher.launches)
        self.assertEqual(1, launcher.shows)
        self.assertTrue(manager.is_running("weather"))
        self.assertTrue(manager.is_visible("weather"))
    ...
    def test_launch_closes_running_peer_in_same_exclusive_group(self) -> None:
        ...
        weather = FakeLauncher(); web_ui = FakeLauncher()
        manager = AppRuntimeManager(config, remote_display=":2")
        manager.register("weather", weather); manager.register("web_ui", web_ui); weather.running = True
    ...
    def test_exclusive_handoff_preserves_preloaded_browser_backend(self) -> None:
        ...
        weather = FakeBrowserDashboardLauncher(); web_ui = FakeLauncher()
        manager = AppRuntimeManager(config, remote_display=":2")
        manager.register("weather", weather); manager.register("web_ui", web_ui); weather.running = True
        manager.launch("web_ui")
    ...
    def test_stop_all_stops_preloaded_app(self) -> None:
        ...
        manager = AppRuntimeManager(self._config(StartupPolicy.PRELOAD), remote_display=":2")
        manager.register("weather", launcher)
        manager._start_background_apps(None); manager.stop_all()
