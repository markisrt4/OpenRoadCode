...
class SDRPPLauncherTest(unittest.TestCase):
    ...
    def test_termux_launch_command_uses_debian_proot(self, which: Mock, _termux: Mock) -> None:
        ...
        self.assertLess(command.index("-u"), command.index("PULSE_SERVER"))
        self.assertLess(command.index("PULSE_SERVER"), command.index("DISPLAY=:1"))
        ...
        self.assertIn("for attempt in $(seq 1 100)", command[-1])
        self.assertIn("PULSE_SERVER=unix:/tmp/runtime-root/pulse/native /usr/bin/pactl info", command[-1])
        self.assertIn("export PULSE_SERVER=unix:/tmp/runtime-root/pulse/native", command[-1])
        self.assertIn("kill -0 $pulse_pid", command[-1])
        self.assertIn("/tmp/orc-sdrpp-pulseaudio.log", command[-1])
