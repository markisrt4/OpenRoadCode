...
class StreamlitLauncher(AppLauncherIf):
    ...
    def __init__(
        ...
        *,
        ...
        self.port = port
        self.log_file = Path(log_file or logging_file_path("openroadcode", "streamlit.log"))
        self.startup_timeout_seconds = startup_timeout_seconds
        self.environment = dict(environment or {})
        self.browser = browser or BrowserKioskLauncher(
            url=f"http://127.0.0.1:{port}",
            process_pattern=f"127.0.0.1:{port}",
            profile_path=Path.home() / "snap" / "chromium" / "common" / f"openroadcode-streamlit-{port}",
            window_class=f"OpenRoadCodeStreamlit{port}",
            exclusive_group=browser_exclusive_group,
            log_file=browser_log_file or logging_file_path("openroadcode", "streamlit-browser.log"),
        )
    ...
    def launch(self, remote_display: str, set_status: StatusCallback = None) -> None:
        _status(set_status, f"Launching Streamlit app: {self.app_path.name}")
        self.prepare()
        self.browser.launch(remote_display, set_status)
        _status(set_status, f"Streamlit dashboard launched on {remote_display}")

    def prepare(self) -> None:
        """Start the Streamlit server without opening its browser."""
        if not self.app_path.is_file():
            raise FileNotFoundError(f"Streamlit application not found: {self.app_path}")
    ...
    def _start_server(self) -> None:
        ...
        try:
            self._process = subprocess.Popen(
                [sys.executable, "-m", "streamlit", "run", str(self.app_path), "--server.headless", "true", "--server.port", str(self.port), "--browser.gatherUsageStats", "false"],
