...
class ExternalWindowManager:
    """Find, position, activate, embed, hide, show, and close external X11 windows."""
    ...
    def reparent(
        ...
        *,
        display: str,
        window_class: str,
        parent_window_id: int | str,
        size: tuple[int, int],
    ) -> str | None:
        """Embed a matching external X11 window inside another X11 window."""
        width, height = size
        if width <= 0 or height <= 0:
            raise ValueError("size values must be positive")
        if not self._tools_available("wmctrl", "xprop", "xdotool"):
            return None
        ...
        parent_id = str(parent_window_id)
        self._run(["wmctrl", "-ir", window_id, "-b", "remove,fullscreen"], environment)
        self._run(
            ["wmctrl", "-ir", window_id, "-b", "remove,maximized_vert,maximized_horz"],
            environment,
        ...
        self._run(["xdotool", "windowreparent", window_id, parent_id], environment)
...
def x11_environment(display: str) -> dict[str, str]:
    ...
    environment.update(
        {"DISPLAY": display, "XDG_SESSION_TYPE": "x11", "GDK_BACKEND": "x11"}
    )
    return environment
