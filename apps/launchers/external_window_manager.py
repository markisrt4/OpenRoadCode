...
class ExternalWindowManager:
    ...
    def hide(self, *, display: str, window_id: str | None) -> bool:
        """Hide an identified external window without terminating its process.

        Window managers do not all treat ``xdotool windowminimize`` identically.
        Request the EWMH hidden state as well so helper browser windows reliably
        disappear under XFCE and other lightweight X11 desktops.
        """
        if window_id is None:
            return False
        environment = x11_environment(display)
        attempted = False
        if self._tools_available("xdotool"):
            self._run(["xdotool", "windowminimize", window_id], environment)
            attempted = True
        if self._tools_available("wmctrl"):
            self._run(["wmctrl", "-ir", window_id, "-b", "add,hidden"], environment)
            attempted = True
        return attempted
