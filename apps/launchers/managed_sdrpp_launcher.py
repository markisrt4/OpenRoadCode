...
class ManagedSDRPPLauncher(SDRPPLauncher):
    ...
    def prepare(self, remote_display: str, set_status: StatusCallback = None) -> None:
        ...
        self.hide(remote_display, set_status)

    def launch(self, remote_display: str, set_status: StatusCallback = None) -> None:
        """Launch SDR++ for normal presentation without preload hiding."""
        super().launch(remote_display, set_status)
    ...
    def _search_window(selector: str, value: str) -> int | None:
        result = subprocess.run(["xdotool", "search", selector, value], capture_output=True, text=True, check=False)
