...
class WebLinuxAudioAnalysisSession(MusicAnalysisUiIf):
 def __init__(self,source:PcmMusicAnalysisSource|None=None,*,music_lighting:MusicLightingController|None=None)->None:
  self.source=source or PcmMusicAnalysisSource(PipeWireAudioCapture());self._lock=threading.RLock();self._state=None;self._ui_state=None;self._running=False;self._error=None;self.presenter=MusicAnalysisPresenter(self.source,music_lighting=music_lighting);self.presenter.attach_ui(self)
 ...
 def set_spectrum_mode(self,value:str)->dict[str,object]:self.presenter.request_spectrum_mode(value);return self.state()
 def state(self)->dict[str,object]:
  ...
  if ui is not None:data["ui"]={"status":ui.status.value,"calibrated":ui.calibrated,"sensitivity":ui.sensitivity,"spectrum_mode":ui.spectrum_mode.value,"error":ui.error}
