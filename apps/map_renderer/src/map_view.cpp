...
#if defined(__linux__)
Display* display=glfwGetX11Display();const Window child=glfwGetX11Window(window);if(display&&child!=0){Window rootReturn=0,childReturn=0;int rootX=0,rootY=0,winX=0,winY=0;unsigned int mask=0;if(XQueryPointer(display,child,&rootReturn,&childReturn,&rootX,&rootY,&winX,&winY,&mask)){clickX=static_cast<double>(winX);clickY=static_cast<double>(winY);}}
#else
