...
void MapView::onMouseClick(GLFWwindow* window,int button,int action,int modifiers){auto* v=static_cast<MapView*>(glfwGetWindowUserPointer(window));if(!v||!v->map||button!=GLFW_MOUSE_BUTTON_LEFT)return;if(action==GLFW_PRESS){v->pressX=v->lastX;v->pressY=v->lastY;v->manualGesturePublished=false;}v->tracking=action==GLFW_PRESS;v->map->setGestureInProgress(v->tracking);if(action==GLFW_RELEASE){const double moved=std::hypot(v->lastX-v->pressX,v->lastY-v->pressY);const double now=glfwGetTime();if(now-v->lastClick<0.4){if(v->manualCameraCallback)v->manualCameraCallback();const mbgl::ScreenCoordinate anchor{v->lastX,v->lastY};v->map->scaleBy(modifiers&GLFW_MOD_SHIFT?0.5:2.0,anchor,mbgl::AnimationOptions{{mbgl::Milliseconds(500)}});}else if(moved<8.0&&v->mapClickCallback){const auto click=v->map->latLngForPixel({v->lastX,v->lastY});const auto edge=v->map->latLngForPixel({v->lastX+48.0,v->lastY});constexpr double earthRadiusM=6378137.0;const double lat1=click.latitude()*M_PI/180.0;const double lat2=edge.latitude()*M_PI/180.0;const double dLat=(edge.latitude()-click.latitude())*M_PI/180.0;const double dLon=(edge.longitude()-click.longitude())*M_PI/180.0;const double h=std::sin(dLat/2.0)*std::sin(dLat/2.0)+std::cos(lat1)*std::cos(lat2)*std::sin(dLon/2.0)*std::sin(dLon/2.0);const double radius=std::max(10.0,2.0*earthRadiusM*std::asin(std::min(1.0,std::sqrt(h))));std::string markerId;double nearestPx=48.0;for(const auto& result:v->poiResults){const auto pixel=v->map->pixelForLatLng({result.latitude,result.longitude});const double distancePx=std::hypot(pixel.x-v->lastX,pixel.y-v->lastY);if(distancePx<=nearestPx){nearestPx=distancePx;markerId=result.id;}}std::cout<<"[map_renderer] map click lat="<<click.latitude()<<" lon="<<click.longitude()<<" radius_m="<<radius<<" marker_id="<<(markerId.empty()?"<none>":markerId)<<" nearest_px="<<nearestPx<<'\n';v->mapClickCallback(click.latitude(),click.longitude(),radius,markerId);}v->lastClick=now;}}
