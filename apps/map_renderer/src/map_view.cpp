...
void MapView::invalidate(){dirty=true;glfwPostEmptyEvent();} void MapView::onWillStartRenderingFrame(){invalidate();} void MapView::setUpdateCallback(std::function<void()> callback){updateCallback=std::move(callback);} void MapView::setPoiSelectedCallback(PoiSelectedCallback callback){poiSelectedCallback=std::move(callback);} void MapView::setManualCameraCallback(ManualCameraCallback callback){manualCameraCallback=std::move(callback);}