...
void MapView::selectPoiAt(double x,double y){if(!map||!rendererFrontend||!rendererFrontend->getRenderer()||!poiSelectedCallback)return;const mbgl::RenderedQueryOptions options{{{"poi-results-marker","poi-results-label","poi-results-glow","poi-labels-food","poi-labels-important","fuel-focus-label","fuel-focus-glow","grocery-focus-label","grocery-focus-glow","food-focus-label","food-focus-glow","transit-focus-label","transit-focus-glow"}}, {}};const double offsets[][2]={{0,0},{-8,0},{8,0},{0,-8},{0,8},{-8,-8},{8,-8},{-8,8},{8,8},{-14,0},{14,0},{0,-14},{0,14}};for(const auto& offset:offsets){const mbgl::ScreenCoordinate point{x+offset[0],y+offset[1]};const auto features=rendererFrontend->getRenderer()->queryRenderedFeatures(point,options);if(features.empty())continue;const auto& f=features.front();std::string name=stringProperty(f,"name:latin");if(name.empty())name=stringProperty(f,"name");if(name.empty())continue;const auto featureCoordinate=pointCoordinate(f);const auto coordinate=featureCoordinate.value_or(map->latLngForPixel(point));const auto brand=stringProperty(f,"brand");auto sourceClass=stringProperty(f,"class");if(sourceClass.empty())sourceClass=stringProperty(f,"category");const auto sourceSubclass=stringProperty(f,"subclass");std::cout<<"[map_renderer] selected POI: "<<name<<'\n';poiSelectedCallback(name,brand,sourceClass,sourceSubclass,coordinate.latitude(),coordinate.longitude());return;}std::cout<<"[map_renderer] POI click miss x="<<x<<" y="<<y<<'\n';}
