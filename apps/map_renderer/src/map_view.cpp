...
PoiSearchResult MapView::searchVisiblePois(const std::string& category)const{PoiSearchResult result;if(!map||!rendererFrontend||!rendererFrontend->getRenderer())return result;const auto topLeft=map->latLngForPixel({0.0,0.0});const auto bottomRight=map->latLngForPixel({static_cast<double>(width),static_cast<double>(height)});const double viewportSouth=std::min(topLeft.latitude(),bottomRight.latitude());const double viewportNorth=std::max(topLeft.latitude(),bottomRight.latitude());const double viewportWest=std::min(topLeft.longitude(),bottomRight.longitude());const double viewportEast=std::max(topLeft.longitude(),bottomRight.longitude());const mbgl::SourceQueryOptions options{{{"poi"}}, {}};const auto features=rendererFrontend->getRenderer()->querySourceFeatures("openroad",options);double south=std::numeric_limits<double>::max(),west=std::numeric_limits<double>::max(),north=std::numeric_limits<double>::lowest(),east=std::numeric_limits<double>::lowest();for(const auto& feature:features){if(!categoryMatches(feature,category))continue;const auto coordinate=pointCoordinate(feature);if(!coordinate)continue;const double lat=coordinate->latitude(),lon=coordinate->longitude();if(lat<viewportSouth||lat>viewportNorth||lon<viewportWest||lon>viewportEast)continue;++result.count;south=std::min(south,lat);north=std::max(north,lat);west=std::min(west,lon);east=std::max(east,lon);}if(result.count>0){result.south=south;result.west=west;result.north=north;result.east=east;}std::cout<<"[map_renderer] POI search category="<<category<<" source_features="<<features.size()<<" matches="<<result.count<<'\n';return result;}
