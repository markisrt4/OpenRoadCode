...
#if defined(__linux__)
void embedInX11Parent(GLFWwindow* window){const char* value=std::getenv("OPENROADCODE_MAP_PARENT_WINDOW");if(!value||!*value)return;char* end=nullptr;const unsigned long parentId=std::strtoul(value,&end,0);if(end==value||*end!='\0'||parentId==0){std::cerr<<"[map_renderer] invalid OPENROADCODE_MAP_PARENT_WINDOW="<<value<<'\n';return;}Display* display=glfwGetX11Display();const Window child=glfwGetX11Window(window);if(!display||child==0){std::cerr<<"[map_renderer] GLFW X11 native window is unavailable\n";return;}XWindowAttributes a{};if(!XGetWindowAttributes(display,static_cast<Window>(parentId),&a)){std::cerr<<"[map_renderer] X11 parent window "<<parentId<<" is unavailable\n";return;}XReparentWindow(display,child,static_cast<Window>(parentId),0,0);XResizeWindow(display,child,static_cast<unsigned int>(a.width),static_cast<unsigned int>(a.height));XMapWindow(display,child);XFlush(display);glfwSetWindowSize(window,a.width,a.height);std::cout<<"[map_renderer] embedded in X11 parent "<<parentId<<" size="<<a.width<<'x'<<a.height<<'\n';}
#endif
}
...
#endif
if(action==GLFW_PRESS){v->pressX=clickX;v->pressY=clickY;v->manualGesturePublished=false;}v->tracking=action==GLFW_PRESS;v->map->setGestureInProgress(v->tracking);if(action==GLFW_RELEASE){const double moved=std::hypot(clickX-v->pressX,clickY-v->pressY);const double now=glfwGetTime();if(now-v->lastClick<0.4){if(v->manualCameraCallback)v->manualCameraCallback();const mbgl::ScreenCoordinate anchor{clickX,clickY};v->map->scaleBy(modifiers&GLFW_MOD_SHIFT?0.5:2.0,anchor,mbgl::AnimationOptions{{mbgl::Milliseconds(500)}});}else if(moved<8.0&&v->mapClickCallback){const auto click=v->map->latLngForPixel({clickX,clickY});const auto edge=v->map->latLngForPixel({clickX+48.0,clickY});constexpr double earthRadiusM=6378137.0;const double lat1=click.latitude()*M_PI/180.0;const double lat2=edge.latitude()*M_PI/180.0;const double dLat=(edge.latitude()-click.latitude())*M_PI/180.0;const double dLon=(edge.longitude()-click.longitude())*M_PI/180.0;const double h=std::sin(dLat/2.0)*std::sin(dLat/2.0)+std::cos(lat1)*std::cos(lat2)*std::sin(dLon/2.0)*std::sin(dLon/2.0);const double radius=std::max(10.0,2.0*earthRadiusM*std::asin(std::min(1.0,std::sqrt(h))));std::string markerId;std::size_t markerIndex=std::numeric_limits<std::size_t>::max();if(v->rendererFrontend&&v->rendererFrontend->getRenderer()){const mbgl::ScreenBox hitBox{{clickX-30.0,clickY-30.0},{clickX+30.0,clickY+30.0}};const auto renderedFeatures=v->rendererFrontend->getRenderer()->queryRenderedFeatures(hitBox,{});for(const auto& feature:renderedFeatures){const auto idIt=feature.properties.find("id");if(idIt==feature.properties.end()||!idIt->second.is<std::string>())continue;const auto candidateId=idIt->second.get<std::string>();for(std::size_t index=0;index<v->poiResults.size();++index){if(v->poiResults[index].id==candidateId){markerId=candidateId;markerIndex=index;break;}}if(!markerId.empty())break;}std::cout<<"[map_renderer] rendered hit features="<<renderedFeatures.size()<<'\n';}std::cout<<"[map_renderer] map click x="<<clickX<<" y="<<clickY<<" lat="<<click.latitude()<<" lon="<<click.longitude()<<" radius_m="<<radius<<" marker_id="<<(markerId.empty()?"<none>":markerId)<<" marker_index="<<(markerIndex==std::numeric_limits<std::size_t>::max()?-1:static_cast<long long>(markerIndex))<<'\n';v->mapClickCallback(click.latitude(),click.longitude(),radius,markerId,markerIndex);}v->lastClick=now;}}
...
void MapView::invalidate(){dirty=true;glfwPostEmptyEvent();} void MapView::onWillStartRenderingFrame(){invalidate();} void MapView::setUpdateCallback(std::function<void()> callback){updateCallback=std::move(callback);} void MapView::setPoiSelectedCallback(PoiSelectedCallback callback){poiSelectedCallback=std::move(callback);} void MapView::setManualCameraCallback(ManualCameraCallback callback){manualCameraCallback=std::move(callback);} void MapView::setMapClickCallback(MapClickCallback callback){mapClickCallback=std::move(callback);}