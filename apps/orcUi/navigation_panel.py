...
_EARTH_BOOTSTRAP_NUDGE_DEG=0.0001
_EARTH_BOOTSTRAP_SETTLE_MS=150
...
class NavigationPanel(tk.Frame):
 def __init__(self,parent:tk.Misc,*,map_request_handler:MapRequestHandlerIf|None=None,on_back:Callable[[],None]|None=None)->None:
  ...
  self._earth_launcher=GoogleEarthLauncher(); self._earth_embedder=X11WindowEmbedder(); self._earth_camera=EarthCdpCameraController(); self._earth_watch_probe=EarthGeolocationWatchProbe(); self._earth_visible=False; self._earth_initialized=False; self._earth_hud_after:str|None=None; self._earth_prime_after:str|None=None; self._earth_last_sent_position:tuple[float,float]|None=None; self._earth_watch_count=0; self._earth_tracking_primed=False; self._earth_tracking_prime_pending=False
 ...
 def destroy(self):
  self._stop_earth_hud(); self._cancel_earth_prime(); self._detach_earth()
 ...
 def _embed_earth(self)->None:
  ...
  self._earth_last_sent_position=None;self._earth_watch_count=0;self._earth_tracking_primed=False;self._earth_tracking_prime_pending=False;self._earth_watch_probe.install();self._earth_visible=True;self._earth_button.configure(text="▣  MAP",bg=GREEN,fg=BG);self._start_earth_hud()
 ...
 def _leave_earth(self)->None:
  self._stop_earth_hud();self._cancel_earth_prime();self._detach_earth();self._earth_guidance.grid_remove();self._earth_hud.grid_remove();self._earth_visible=False;self._earth_button.configure(text="◉  EARTH",bg=BLUE,fg="white");self._shortcut_status.set("MapLibre")
 def _toggle_earth(self):
  try:
   ...
   self._earth_visible=False;self._stop_earth_hud();self._cancel_earth_prime();self._detach_earth();self._earth_guidance.grid_remove();self._earth_hud.grid_remove()
 ...
 def _cancel_earth_prime(self)->None:
  if self._earth_prime_after is not None:
   try:self.after_cancel(self._earth_prime_after)
   except tk.TclError:pass
   self._earth_prime_after=None
  self._earth_tracking_prime_pending=False
 def _refresh_earth_tracking_watch(self)->None:
  ...
  if count>self._earth_watch_count:
   self._earth_watch_count=count;self._earth_tracking_primed=False;self._earth_tracking_prime_pending=False;self._earth_last_sent_position=None
  if count>0 and not self._earth_tracking_primed and not self._earth_tracking_prime_pending:self._prime_earth_tracking()
 def _prime_earth_tracking(self)->None:
  ...
  nudge_lat=max(-90.0,min(90.0,lat+_EARTH_BOOTSTRAP_NUDGE_DEG))
  view=EarthCameraView(latitude_deg=nudge_lat,longitude_deg=lon,heading_deg=None if track is None else math.degrees(track)%360.0,speed_m_s=None if speed is None else max(0.0,speed))
  if not self._earth_camera.set_view(view):self._shortcut_status.set("Earth GPS bridge unavailable");return
  self._earth_tracking_prime_pending=True;self._shortcut_status.set("Earth GPS tracking starting");self._earth_prime_after=self.after(_EARTH_BOOTSTRAP_SETTLE_MS,self._finish_earth_tracking_prime)
 def _finish_earth_tracking_prime(self)->None:
  self._earth_prime_after=None;self._earth_tracking_prime_pending=False
 ...
 def _send_earth_position(self,*,force:bool=False)->bool:
  ...
  if self._earth_camera.set_view(EarthCameraView(latitude_deg=lat,longitude_deg=lon,heading_deg=None if track is None else math.degrees(track)%360.0,speed_m_s=None if speed is None else max(0.0,speed))):self._earth_last_sent_position=(lat,lon);return True
