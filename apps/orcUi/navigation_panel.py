...
class NavigationPanel(tk.Frame):
    ...
    def _pan(self,up:float,right:float)->None:
        self.set_follow_enabled(False); self._map_host.update_idletasks(); self._request_handler.request_pan_screen(right_px=right*max(48,self._map_host.winfo_width()*0.25),up_px=up*max(48,self._map_host.winfo_height()*0.25))

    def _change_zoom(self,delta:float)->None:
        self._zoom_level=max(1,min(22,self._zoom_level+delta)); self._zoom_text.set(f"{self._zoom_level:.1f}"); self.set_follow_enabled(False); self._request_handler.request_zoom(self._zoom_level)
