...
class NavigationPanel(tk.Frame):
    def __init__(self,parent:tk.Misc,*,map_request_handler:MapRequestHandlerIf|None=None,on_back:Callable[[],None]|None=None)->None:
        ...
        self._earth_launcher=GoogleEarthLauncher(); self._earth_visible=False; self._earth_initialized=False
    ...
    def _build(self)->None:
        ...
        self._earth_button=tk.Button(bar,text="◉  EARTH",command=self._toggle_earth,bg=BLUE,fg="#ffffff",activebackground=GREEN,activeforeground="#05090d",relief=tk.FLAT,highlightthickness=2,highlightbackground="#5bbcff",font=("Sans",9,"bold"),width=11,height=1,padx=5,pady=1); self._earth_button.pack(side=tk.RIGHT,padx=(7,2),pady=2)
    ...
    def _display(self)->str: return os.environ.get("DISPLAY",":1")
    def _set_earth_layout(self,enabled:bool)->None:
        if enabled: self._controls.grid_remove()
        else: self._controls.grid()
        self._body.update_idletasks()
    def _prepare_first_earth_launch(self)->None:
        ...
        if current_position is not None:
            latitude=math.degrees(current_position.latitude_rad); longitude=math.degrees(current_position.longitude_rad)
            self._earth_launcher.set_location(latitude,longitude)
            self._shortcut_status.set(f"Earth {latitude:.5f}, {longitude:.5f}")
        else:
            self._shortcut_status.set("Earth: waiting for GPS; using default")
        self._earth_initialized=True
    def _toggle_earth(self)->None:
        try:
            self._map_host.update_idletasks()
            if not self._earth_visible:
                self._set_earth_layout(True); self._prepare_first_earth_launch()
                position=(self._map_host.winfo_rootx(),self._map_host.winfo_rooty()); size=(max(1,self._map_host.winfo_width()),max(1,self._map_host.winfo_height()))
            ...
            if self._earth_visible: self._earth_button.configure(text="▣  MAP",bg=GREEN,fg="#05090d",highlightbackground="#b8f55f")
            else:
                self._set_earth_layout(False); self._earth_button.configure(text="◉  EARTH",bg=BLUE,fg="#ffffff",highlightbackground="#5bbcff"); self._shortcut_status.set("MapLibre")
    ...
    def _is_dark_theme(self)->bool:
        color=BG.lstrip("#"); red,green,blue=(int(color[index:index+2],16) for index in (0,2,4)); return (red*299+green*587+blue*114)/1000<128
