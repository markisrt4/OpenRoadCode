...
class OrcUiRadioControl:
    ...
    def __init__(self, config_path: str | Path | None = None, *, catalog: OrcUiRadioProfileCatalog | None = None, client: RigctlClient | None = None) -> None:
        self._catalog = catalog or OrcUiRadioProfileCatalog()
        if not self._catalog.profiles:
            raise ValueError("No usable radio profiles are installed")
    ...
    @property
    def catalog(self) -> OrcUiRadioProfileCatalog: return self._catalog
    @property
    ...
    def state(self) -> OrcUiRadioState:
        ...
        if self._radio.is_started and self._profile_key == "fm_radio":
            try: rds = self._radio.get_rds()
            except (OSError, RuntimeError, ValueError): rds = None
    ...
    def select_profile(self, profile_key: str, *, start: bool = True) -> OrcUiRadioState:
        ...
        if was_started: self._radio.stop()
    ...
    def next_preset(self) -> OrcUiRadioState: self._ensure_started(); self._radio.next_preset(); return self.state
    def previous_preset(self) -> OrcUiRadioState: self._ensure_started(); self._radio.previous_preset(); return self.state
    def tune_up(self) -> OrcUiRadioState: self._ensure_started(); self._radio.frequency_up(); return self.state
    def tune_down(self) -> OrcUiRadioState: self._ensure_started(); self._radio.frequency_down(); return self.state
    ...
    def _build_controller(self, config) -> RadioController:
        ...
        try:
            ...
            for item in profile.presets:
                if (item.label, item.frequency_hz) not in shipped:
        ...
        if config.radio_range is not None:
            radio_range = RadioRange(config.radio_range.min_frequency_hz, config.radio_range.max_frequency_hz, config.radio_range.start_frequency_hz)
    ...
    def _matching_preset(self, frequency_hz: int) -> RadioPreset | None:
        for preset in self._radio.presets:
            if preset.frequency_hz == frequency_hz: return preset
        return None
