...
class MediaPanel(tk.Frame):
    ...
    def close(self)->None:
        ...
        self._closed=True; self._cancel_spotify_refresh(); self._spotify_library_generation+=1
    ...
    def _load_spotify_collection(self,title,cached_loader,network_loader)->None:
        ...
        if cached is not None:
            self._render_spotify_collection(title,cached,generation); return
    ...
    def _render_spotify_collection(self,title,tracks,generation)->None:
        """Render Spotify tracks as compact three-column artwork cards."""
        if self._closed or self._view_host is None or generation!=self._spotify_library_generation:return
        for child in self._view_host.winfo_children():child.destroy()
        self._spotify_library_images=[]; toolbar=tk.Frame(self._view_host,bg=BG); toolbar.pack(fill=tk.X,padx=6,pady=(0,4)); tk.Button(toolbar,text="‹ NOW PLAYING",command=self.show_spotify,bg=ACTIVE,fg=TEXT,relief=tk.FLAT,bd=0,font=("Sans",9,"bold"),padx=10,pady=5).pack(side=tk.LEFT); tk.Label(toolbar,text=title,bg=BG,fg=TEXT,font=("Sans",11,"bold")).pack(side=tk.LEFT,padx=12); list_host=tk.Frame(self._view_host,bg=BG); list_host.pack(fill=tk.BOTH,expand=True,padx=6,pady=4); [list_host.grid_columnconfigure(column,weight=1,uniform="spotify-track") for column in range(LIBRARY_COLUMNS)]
    ...
    def _spotify_track_card(self,parent,track,row,column,generation)->None:
        """Build one compact library card and populate its artwork asynchronously."""
    ...
    def _load_track_art(self,url,label,generation)->None:
        """Resolve a small album thumbnail through the shared persistent image cache."""
        def worker():
    ...
    def _clear_view(self):
        self._stop_embedded_browser(); self._cancel_spotify_refresh(); self._spotify_panel=None; self._spotify_mode_buttons={}; self._spotify_mode_status=None; self._spotify_library_generation+=1; self._spotify_library_images=[]
