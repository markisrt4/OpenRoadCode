...
class MediaPanel(tk.Frame):
    ...
    def set_theme_mode(self, theme_mode: ThemeMode) -> None:
        """Apply the ORC theme to future and currently embedded browser sessions."""
    ...
    def show_youtube(self) -> None:
        self._show_embedded_browser(service="YouTube", subtitle="Embedded YouTube kiosk", window_class=YOUTUBE_WINDOW_CLASS, launch=self._launch_youtube)

    def show_netflix(self) -> None:
        self._show_embedded_browser(service="Netflix", subtitle="Embedded Netflix kiosk", window_class=NETFLIX_WINDOW_CLASS, launch=self._launch_netflix)
    ...
    def _launch_youtube(self, position: tuple[int, int], size: tuple[int, int]) -> None:
        ...
        self._youtube_player.play("https://www.youtube.com/", display=self._display, window_position=position, window_size=size)
    ...
    def _launch_netflix(self, position: tuple[int, int], size: tuple[int, int]) -> None:
        ...
        self._netflix_player.play("https://www.netflix.com/browse", display=self._display, window_position=position, window_size=size)
    ...
    def _show_embedded_browser(
        ...
        *,
        ...
        try:
            ...
            self.update_idletasks()
            self._browser_embedder.embed(0, int(host.winfo_id()), size[0], size[1], window_class=window_class)
    ...
    def _build_shell(self) -> None:
        ...
        self._back_button = tk.Button(self._header, text="‹ HOME", command=self._on_back, bg=PANEL, fg=TEXT, activebackground=ACTIVE, activeforeground=TEXT, relief=tk.FLAT, bd=0, font=("Sans", 10, "bold"), padx=10, pady=8)
        ...
        self._media_back = tk.Button(self._header, text="ALL MEDIA", command=self.show_hub, bg=ACTIVE, fg=TEXT, activebackground=BORDER, activeforeground=TEXT, relief=tk.FLAT, bd=0, font=("Sans", 9, "bold"), padx=12, pady=7)
    ...
    def _set_title(self, title: str, subtitle: str, *, show_media_back: bool = False) -> None:
        ...
        self._media_back.grid() if show_media_back else self._media_back.grid_remove()
    ...
    def _refresh_spotify(self) -> None:
        ...
        threading.Thread(target=self._load_spotify_state, args=(presenter, generation), name="orcui-spotify-state", daemon=True).start()
    ...
    def _media_card(self, parent: tk.Widget, glyph: str, title: str, category: str, subtitle: str, detail: str, action: str, accent: str, command: Callable[[], None]) -> tk.Frame:
        ...
        tk.Label(body, text=detail, bg=PANEL, fg=MUTED, font=("Sans", 9), justify=tk.LEFT, wraplength=220).pack(anchor="w")
        button = tk.Button(body, text=f"{action}   ›", command=command, bg=accent, fg=BG, activebackground=accent, activeforeground=BG, relief=tk.FLAT, bd=0, font=("Sans", 9, "bold"), padx=12, pady=9, cursor="hand2")
    ...
    def _show_error(self, service: str, error: Exception) -> None:
        ...
        tk.Label(self._view_host, text=f"{service} unavailable\n\n{error}", bg=BG, fg=TEXT, font=("Sans", 15, "bold"), justify=tk.CENTER, wraplength=650).place(relx=0.5, rely=0.5, anchor="center")
