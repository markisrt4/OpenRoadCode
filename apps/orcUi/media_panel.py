...
class MediaPanel(tk.Frame):
    ...
    def set_theme_mode(self, theme_mode: ThemeMode) -> None:
        ...
        if active_browser == "youtube":
            self.show_youtube()
        elif active_browser == "netflix":
            self.show_netflix()
    ...
    def show_hub(self) -> None:
        ...
        hero=tk.Frame(self._view_host,bg=BG); hero.pack(fill=tk.X,padx=12,pady=(2,4))
        tk.Label(hero,text="YOUR MEDIA",bg=BG,fg=TEXT,font=("Sans",22,"bold")).pack(anchor="w")
        tk.Label(hero,text="Pick a service and go. No launcher maze required.",bg=BG,fg=MUTED,font=("Sans",10)).pack(anchor="w",pady=(1,0))
        grid=tk.Frame(self._view_host,bg=BG); grid.pack(fill=tk.BOTH,expand=True,padx=6,pady=(4,8))
        for column in range(3):grid.grid_columnconfigure(column,weight=1,uniform="media")
        grid.grid_rowconfigure(0,weight=1)
        cards=(("♫","SPOTIFY","MUSIC","Now playing","Artwork, lyrics, playback controls and music video.","OPEN PLAYER",ACCENT_GREEN,self.show_spotify),("▶","YOUTUBE","VIDEO","Watch anything","Open YouTube inside the ORC media surface.","OPEN YOUTUBE",ACCENT_RED,self.show_youtube),("N","NETFLIX","STREAM","Continue watching","Open Netflix inside ORC using your retained browser profile.","OPEN NETFLIX",ACCENT_BLUE,self.show_netflix))
        for column,card in enumerate(cards):self._media_card(grid,*card).grid(row=0,column=column,sticky="nsew",padx=6,pady=4)
    ...
    def show_spotify(self) -> None:
        ...
        try:
            ...
            video_controller=MusicVideoController(spotify_controller=self._spotify_service.controller,music_video=YouTubeMusicVideo(port=MUSIC_VIDEO_PORT,fullscreen=True,software_rendering=target is RuntimeTarget.LINUX_DEV))
            panel=SpotifyPlaybackPanel(self._view_host,music_video_controller=video_controller,image_cache=ImageCache(max_entries=64),lyrics_client=LrclibLyricsClient(),theme=_orc_spotify_theme())
            panel.set_playback_request_handler(self._spotify_service); panel.set_track_request_handler(self._spotify_service); panel.set_seek_request_handler(self._spotify_service); panel.set_volume_request_handler(self._spotify_service)
            panel.pack(fill=tk.BOTH,expand=True,padx=4,pady=4)
            self._spotify_video_controller=video_controller; self._spotify_panel=panel
    ...
    def _launch_youtube(self,position,size):
        self._youtube_player=YouTubePlayer(software_rendering=detect_runtime_target() is RuntimeTarget.LINUX_DEV,dark_mode=self._theme_mode is ThemeMode.DARK); self._youtube_player.play("https://www.youtube.com/",display=self._display,window_position=position,window_size=size); self._active_browser="youtube"
    def _launch_netflix(self,position,size):
        self._netflix_player=NetflixPlayer(software_rendering=detect_runtime_target() is RuntimeTarget.LINUX_DEV,dark_mode=self._theme_mode is ThemeMode.DARK); self._netflix_player.play("https://www.netflix.com/browse",display=self._display,window_position=position,window_size=size); self._active_browser="netflix"
    ...
    def _build_shell(self):
        self.grid_columnconfigure(0,weight=1); self.grid_rowconfigure(1,weight=1); self._header=tk.Frame(self,bg=PANEL,highlightthickness=1,highlightbackground=BORDER); self._header.grid(row=0,column=0,sticky="ew",padx=2,pady=(2,6)); self._header.grid_columnconfigure(1,weight=1)
        self._back_button=tk.Button(self._header,text="‹ HOME",command=self._on_back,bg=PANEL,fg=TEXT,activebackground=ACTIVE,activeforeground=TEXT,relief=tk.FLAT,bd=0,font=("Sans",10,"bold"),padx=10,pady=8); self._back_button.grid(row=0,column=0,rowspan=2,sticky="nsw",padx=(6,10),pady=5)
        self._title_label=tk.Label(self._header,text="MEDIA",bg=PANEL,fg=TEXT,font=("Sans",16,"bold")); self._title_label.grid(row=0,column=1,sticky="sw",pady=(7,0)); self._subtitle_label=tk.Label(self._header,text="",bg=PANEL,fg=MUTED,font=("Sans",9)); self._subtitle_label.grid(row=1,column=1,sticky="nw",pady=(0,7)); self._media_back=tk.Button(self._header,text="ALL MEDIA",command=self.show_hub,bg=ACTIVE,fg=TEXT,activebackground=BORDER,activeforeground=TEXT,relief=tk.FLAT,bd=0,font=("Sans",9,"bold"),padx=12,pady=7); self._media_back.grid(row=0,column=2,rowspan=2,padx=8,pady=6); self._view_host=tk.Frame(self,bg=BG); self._view_host.grid(row=1,column=0,sticky="nsew")
    ...
    def _refresh_spotify(self):
        ...
        self._spotify_refresh_job=self.after(1000,self._refresh_spotify)
