...
class OffRoadPanel(tk.Frame):
    ...
    def __init__(self, parent: tk.Misc, *, on_back: Callable[[], None], position: PositionPresentationState | None = None, attitude: AttitudePresentationState | None = None, show_header: bool = True) -> None:
        ...
        if show_header:
            ...
            tk.Button(header, text="‹ HOME", command=on_back, bg="#101820", fg=TEXT, relief=tk.FLAT, bd=0, font=("Sans", 10, "bold"), padx=14, pady=6).pack(side=tk.LEFT)
            tk.Label(header, text="OFF-ROAD", fg=YELLOW, bg=BG, font=("Sans", 13, "bold")).pack(side=tk.LEFT, padx=14)
            tk.Label(header, text="NAVIGATION + ATTITUDE", fg=MUTED, bg=BG, font=("Monospace", 9)).pack(side=tk.RIGHT, padx=4)
        ...
        tk.Label(compass, text="HEADING", fg=YELLOW, bg=PANEL, font=("Sans", 9, "bold")).pack(pady=(8, 0))
        ...
        self._heading_label = tk.Label(compass, text="--°", fg=TEXT, bg=PANEL, font=("Sans", 18, "bold"))
        ...
        tk.Label(attitude_card, text="ATTITUDE", fg=BLUE, bg=PANEL, font=("Sans", 9, "bold")).pack(pady=(8, 0))
        self._attitude_canvas = tk.Canvas(attitude_card, width=250, height=100, bg=PANEL, highlightthickness=0)
        self._attitude_canvas.pack(expand=True)
        values = tk.Frame(attitude_card, bg=PANEL)
        ...
        tk.Label(position_card, text="POSITION", fg=GREEN, bg=PANEL, font=("Sans", 10, "bold")).pack(anchor="w", padx=16, pady=(14, 8))
    ...
    def _paint_compass(self, heading: float | None) -> None:
        ...
        for label, degrees in (("N", 0), ("E", 90), ("S", 180), ("W", 270)):
            ...
            c.create_text(cx + 36*math.cos(angle), cy + 36*math.sin(angle), text=label, fill=YELLOW if label == "N" else MUTED, font=("Sans", 9, "bold"))
        if heading is None:
            return
        angle = math.radians(heading - 90)
        c.create_line(cx, cy, cx + 30 * math.cos(angle), cy + 30 * math.sin(angle), fill=YELLOW, width=4, arrow=tk.LAST)
    ...
    def _paint_attitude(self, pitch: float | None, roll: float | None) -> None:
        ...
        dx, dy = 90 * math.cos(angle), 90 * math.sin(angle)
