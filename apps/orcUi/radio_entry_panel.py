...
class RadioEntryPanel(tk.Frame):
    ...
    def __init__(self, parent: tk.Misc, *, radio_application: RadioApplicationServiceIf, streaming_radio: StreamingRadioController, theme: ThemeBundle, embedder: X11WindowEmbedder | None = None) -> None:
        self._theme = theme
        ui = theme.ui
    ...
    def _build_choice_buttons(self) -> None:
        ...
        streaming_card, self._streaming_button = self._build_source_card(parent=self._chooser, title="STREAMING RADIO", eyebrow="INTERNET AUDIO", description="Browse local and regional streams with artwork, favorites and playback.", features="LOCAL  •  REGIONAL  •  FAVORITES", action_text="BROWSE STATIONS  ›", accent=ui.accent_primary, icon_kind="stream", command=self._show_streaming_radio)
        streaming_card.grid(row=0, column=1, sticky="nsew", padx=(6, 12), pady=(18, 12))
    ...
    def _build_source_card(self, *, parent: tk.Misc, title: str, eyebrow: str, description: str, features: str, action_text: str, accent: str, icon_kind: str, command: Callable[[], None]) -> tuple[tk.Frame, tk.Button]:
        ui = self._theme.ui
        ...
        icon = tk.Canvas(heading, width=64, height=64, bg=ui.surface, highlightthickness=0, bd=0)
        icon.grid(row=0, column=0, rowspan=2, sticky="w", padx=(0, 12))
        self._draw_source_icon(icon, icon_kind=icon_kind, accent=accent)
        tk.Label(heading, text=eyebrow, bg=ui.surface, fg=accent, font=("Sans", 9, "bold"), anchor="w").grid(row=0, column=1, sticky="sw", pady=(8, 2))
        tk.Label(heading, text=title, bg=ui.surface, fg=ui.text, font=("Sans", 18, "bold"), anchor="w").grid(row=1, column=1, sticky="nw")
        body = tk.Frame(card, bg=ui.surface)
        body.grid(row=2, column=0, sticky="nsew", padx=26, pady=(8, 14))
        body.grid_columnconfigure(0, weight=1)
        body.grid_rowconfigure(0, weight=1)
        tk.Label(body, text=description, bg=ui.surface, fg=ui.text_muted, font=("Sans", 11), justify=tk.LEFT, anchor="nw", wraplength=390).grid(row=0, column=0, sticky="new")
        tk.Label(body, text=features, bg=ui.surface, fg=ui.text, font=("Sans", 9, "bold"), anchor="w").grid(row=1, column=0, sticky="ew", pady=(14, 8))
        button = tk.Button(body, text=action_text, command=command, bg=ui.control_background, fg=accent, activebackground=ui.control_active, activeforeground="#ffffff", relief=tk.FLAT, bd=0, highlightthickness=1, highlightbackground=ui.border, font=("Sans", 11, "bold"), padx=16, pady=10, cursor="hand2")
    ...
    def _show_streaming_radio(self) -> None:
        ...
        if self._streaming_page is None or not self._streaming_page.winfo_exists():
            self._streaming_page = PersistentStreamingRadioPanel(self, directory=RadioBrowserDirectory(timeout_s=10.0), controller=self._streaming_radio, favorites=StreamingRadioFavorites(), on_back=self._show_chooser)
