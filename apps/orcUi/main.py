...
class OrcUiApp:
    def __init__(self) -> None:
        ...
        self._context_rail=None; self._home_map_panel=None; self._navigation_panel=None; self._radio_panel=None; self._radio_embedder=X11WindowEmbedder(); self._vehicle_panel=None; self._offroad_panel=None
        self._map_renderer=MapRendererLauncher(); self._vehicle_state=VehiclePresentationState(); self._position_state=PositionPresentationState(); self._attitude_state=AttitudePresentationState(); self._volume=20; self._volume_label:tk.Label; self._closing=False
    ...
    def _shutdown_system(self):
        ...
        self._map_renderer.stop(); self._dispatcher.close(); subprocess.Popen(command); self._shutdown()
    ...
    def _toggle_theme(self): self._theme_mode=toggle(self._theme_mode); install_map_style(self._theme_mode); sync_sdrpp_theme("Light" if self._theme_mode is ThemeMode.LIGHT else "Dark"); apply_tk_theme(self._root,self._theme_mode); self._theme_button.configure(text=toggle_label(self._theme_mode)); self._paint_nav(); self._reload_active_map()
    ...
    def _select_nav(self,name):
        self._active_nav=name; self._paint_nav(); {"HOME":self._show_home,"NAVIGATION":self._show_navigation_panel,"RADIO":self._show_radio_panel,"VEHICLE":self._show_vehicle_panel}.get(name,lambda:self._show_placeholder(name))()
    def _paint_nav(self):
    ...
    def _clear_content(self):
        ...
        self._context_rail=self._home_map_panel=self._navigation_panel=self._radio_panel=self._vehicle_panel=self._offroad_panel=None
    ...
    def _show_navigation_panel(self): self._clear_content(); self._active_nav="NAVIGATION"; self._paint_nav(); self._navigation_panel=NavigationPanel(self._content,on_back=self._show_home); self._navigation_panel.pack(fill=tk.BOTH,expand=True); self._root.update_idletasks(); self._start_map_renderer(self._navigation_panel.map_host_window_id)
