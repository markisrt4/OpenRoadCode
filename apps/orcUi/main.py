...
from apps.orcUi.music_visualizer_panel import MusicVisualizerPanel, VisualizerFrame
...
class OrcUiApp:
    def __init__(self) -> None:
        ...
        self._context_rail=None; self._home_map_panel=None; self._navigation_panel=None; self._radio_panel=None; self._radio_embedder=X11WindowEmbedder(); self._vehicle_panel=None; self._offroad_panel=None; self._media_panel=None; self._music_visualizer_panel=None; self._music_visualizer_session=None
        self._music_visualizer_frames: queue.SimpleQueue[VisualizerFrame] = queue.SimpleQueue(); self._music_visualizer_poll_job=None
    ...
    def _shutdown(self):
        ...
        self._closing=True; self._map_renderer.stop(); self._dispatcher.close(); self._spotify_local_player.close(); self._spotify_service.close(); self._close_music_visualizer()
    ...
    def _build_side_nav(self):
        ...
        for item in ["HOME","NAVIGATION","RADIO","MEDIA","VISUALIZER","VEHICLE","LIGHTING","CONTROLS","SETTINGS"]:
    ...
    def _restart_ui(self): self._map_renderer.stop(); self._dispatcher.close(); self._spotify_local_player.close(); self._spotify_service.close(); self._close_music_visualizer(); os.execv(sys.executable,[sys.executable,"-m","apps.orcUi"])
    def _shutdown_system(self):
        command=["systemctl","poweroff"] if shutil.which("systemctl") else (["loginctl","poweroff"] if shutil.which("loginctl") else None)
        if command is None:return
        self._map_renderer.stop(); self._dispatcher.close(); self._spotify_local_player.close(); self._spotify_service.close(); self._close_music_visualizer(); subprocess.Popen(command); self._shutdown()
    ...
    def _select_nav(self,name):
        self._active_nav=name; self._paint_nav(); {"HOME":self._show_home,"NAVIGATION":self._show_navigation_panel,"RADIO":self._show_radio_panel,"MEDIA":self._show_media_panel,"VISUALIZER":self._show_music_visualizer_panel,"VEHICLE":self._show_vehicle_panel}.get(name,lambda:self._show_placeholder(name))()
    def _paint_nav(self):
    ...
    def _show_music_visualizer_panel(self):
        self._clear_content(); self._active_nav="VISUALIZER"; self._paint_nav(); source=selected_music_visualizer_source(); simulate=source is MusicVisualizerSource.SIMULATED; self._music_visualizer_panel=MusicVisualizerPanel(self._content,on_back=self._show_home,simulate=simulate); self._music_visualizer_panel.pack(fill=tk.BOTH,expand=True)
        if not simulate:
            self._music_visualizer_session=create_music_visualizer_session(self._queue_music_visualizer_frame,source=source)
            if self._music_visualizer_session is not None:
                try:self._music_visualizer_session.start()
                except (OSError,RuntimeError) as e:print(f"WARNING: music visualizer audio: {type(e).__name__}: {e}"); self._music_visualizer_session=None
            self._music_visualizer_poll_job=self._root.after(16,self._poll_music_visualizer_frames)
    def _queue_music_visualizer_frame(self,frame): self._music_visualizer_frames.put(frame)
    def _poll_music_visualizer_frames(self):
        self._music_visualizer_poll_job=None; panel=self._music_visualizer_panel
        if panel is None or not panel.winfo_exists():return
        latest=None
        while True:
            try:latest=self._music_visualizer_frames.get_nowait()
            except queue.Empty:break
        if latest is not None:panel.set_analysis_frame(latest)
        if self._music_visualizer_session is not None:self._music_visualizer_poll_job=self._root.after(16,self._poll_music_visualizer_frames)
    def _close_music_visualizer(self):
        if self._music_visualizer_poll_job is not None:
            try:self._root.after_cancel(self._music_visualizer_poll_job)
            except tk.TclError:pass
            self._music_visualizer_poll_job=None
        if self._music_visualizer_session is not None:self._music_visualizer_session.close(); self._music_visualizer_session=None
        if self._music_visualizer_panel is not None and self._music_visualizer_panel.winfo_exists():self._music_visualizer_panel.close()
        self._music_visualizer_panel=None
        while True:
            try:self._music_visualizer_frames.get_nowait()
            except queue.Empty:break
