...
class Latest:
    ...
    def set(self, topic, message):
        with self.lock:
            if topic==POSITION_STATE_TOPIC: self.position=message
            elif topic==MOTION_STATE_TOPIC: self.motion=message
            self.error=None
    def snapshot(self):
...
def receiver(endpoint,latest):
    sub=ZeroMqSubscriber(endpoint); sub.subscribe(POSITION_STATE_TOPIC); sub.subscribe(MOTION_STATE_TOPIC)
    try:
        while True:
            topic,payload=sub.receive()
            latest.set(topic, decode_position_state(payload) if topic==POSITION_STATE_TOPIC else decode_motion_state(payload))
    except Exception as e:
        with latest.lock: latest.error=str(e)
    finally: sub.close()
...
def draw_motion(w,row,col,motion,metric):
    ...
    if motion is None:
        safe(w,row+1,col,"Waiting for motion estimator..."); return
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--endpoint",default="tcp://127.0.0.1:5557"); a=ap.parse_args(); latest=Latest(); threading.Thread(target=receiver,args=(a.endpoint,latest),daemon=True).start()
    try:curses.wrapper(run,a.endpoint,latest)
