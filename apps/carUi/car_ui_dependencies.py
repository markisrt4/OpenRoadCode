...
from controllers.navigation import NavigationControllerIf, PositionSourceIf, UnconfiguredNavigationController
...
class CarUiDependencies:
    ...
    music_visualizer: MusicVisualizerRuntime
    media_display: str | None = None
    navigation_controller: NavigationControllerIf = field(default_factory=UnconfiguredNavigationController)
    ...
    def close(self) -> None:
        ...
        for index,encoder in enumerate(self.rotary_encoders):self._close_resource(f"rotary encoder {index}",encoder.stop)
        for index,keyboard in enumerate(self.keyboards):self._close_resource(f"keyboard {index}",keyboard.close)
        for index,button in enumerate(self.push_buttons):self._close_resource(f"pushbutton {index}",button.stop)
        self._close_resource("music visualizer",self.music_visualizer.close)
        self._close_resource("Car UI runtime",self.runtime.close)
        self._close_resource("Spotify music video",self.spotify_music_video_controller.stop_video)
        ...
        if self.vehicle_state_source is not None:self._close_resource("vehicle telemetry source",self.vehicle_state_source.disconnect)

    @staticmethod
    def _close_resource(name:str,close:Callable[[],object])->None:
        try:close()
        except Exception:LOGGER.exception("Failed to close %s",name)
