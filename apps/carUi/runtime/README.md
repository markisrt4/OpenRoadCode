...
```mermaid
flowchart TD
    config["runtime.toml"] --> parser["RuntimeConfigParser"] --> factory["CarUiRuntimeFactory"] --> runtime["CarUiRuntime"]
    runtime --> rotaryCfg["RotaryEncoderConfig"] --> rotaryRuntime["RotaryEncoderRuntime"]
    rotaryRuntime --> encoders["RotaryEncoderIf[]"]
    rotaryRuntime --> volume["volume_index"]
    runtime --> keyboardCfg["KeyboardConfig"] --> keyboard["KeyboardReaderIf"]
    runtime --> buttonCfg["PushButtonConfig"] --> button["RpiGpioPushButton"]
    runtime --> radioRegistry["RadioRuntimeRegistry"]
    radioRegistry --> fm["fm_radio"]
    radioRegistry --> airband["airband"]
    radioRegistry --> scanner["scanner bands"]
    runtime --> adsb["ADSBLauncher"]
    runtime --> weather["WeatherDashLauncher"]
    runtime --> sdr["SDRResourceManager"]
    ...
    class parser,factory orcController;
    class runtime,rotaryRuntime,radioRegistry orcService;
    class rotaryCfg,keyboardCfg,buttonCfg orcMessage;
    class encoders,keyboard,button,sdr orcAdapter;
    class volume,fm,airband,scanner,adsb,weather orcApp;
```
