...
from controllers.lyrics import LyricsResult, LrclibLyricsClient
...
class SpotifyPanel(tk.Frame):
    ...
    def __init__(
        ...
        *,
        ...
        self._lyrics_result: LyricsResult | None = None
    ...
    def _apply_state(self, state: SpotifyState) -> None:
        ...
        self._update_album_art(state.album_art_url)
        self._update_lyrics(state)

    def _update_lyrics(self, state: SpotifyState | None) -> None:
        if (
            state is None
            or not state.track_name
            or not state.artist_name
        ):
        ...
        self._render_lyrics(
            progress_ms=state.progress_ms or 0,
    ...
    def _adjust_volume(self, delta: int) -> None:
        ...
        threading.Thread(
            target=self._set_volume_worker,
