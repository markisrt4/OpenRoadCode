...
class TkCarUiScreenFactory:
    ...
    def create_screens(self, dependencies: CarUiDependencies, on_frequency_changed: Callable[[int], None], dispatch: Callable[[Callable[[], None]], None]) -> CarUiScreens:
        ...
        music_visualizer = MusicVisualizerScreen(self._host, create_menu_tile=self._create_menu_tile, back_action=lambda: self._show_menu("media"))
        offroad_dashboard = OffroadDashboardScreen(self._host, controller=dependencies.navigation_controller, create_menu_tile=self._create_menu_tile, back_action=lambda: self._show_menu("gauges"))
        vehicle_gauges = VehicleGaugesScreen(self._host, source=getattr(dependencies, "vehicle_state_source", None), create_menu_tile=self._create_menu_tile, back_action=lambda: self._show_menu("gauges"))
        return CarUiScreens(aircraft=aircraft, weather=weather, lighting=lighting, fm_radio=fm_radio, scanner=scanner, spotify=spotify, netflix=netflix, youtube=youtube, music_visualizer=music_visualizer, offroad_dashboard=offroad_dashboard, vehicle_gauges=vehicle_gauges)
