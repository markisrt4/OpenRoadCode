...
from controllers.lighting.lighting_types import CustomPatternMode, LightingConnectionStatus, LightingState, RgbColor
from hardware_io.bluetooth import BleGattState, BleGattStatus, BleGattTransportIf
...
class LedDmxController(LightingControllerIf):
    ...
    def __init__(self, transport: BleGattTransportIf) -> None:
        ...
        self._thread = threading.Thread(target=self._run_loop, name="LedDmxControllerLoop", daemon=True)
        self._thread.start()
    ...
    def set_power(self, enabled: bool) -> Future[None]:
        return self._command(LedDmxProtocol.power(enabled), power_enabled=bool(enabled))
    ...
    def set_brightness(self, percent: int) -> Future[None]:
        ...
        return self._command(LedDmxProtocol.brightness(value), brightness_percent=value)

    def set_color_temperature(self, percent: int) -> Future[None]:
        value = max(0, min(percent, 100))
        return self._command(LedDmxProtocol.color_temperature(value), color_temperature_percent=value)

    def set_pattern(self, pattern_index: int) -> Future[None]:
        value = max(0, min(pattern_index, 210))
        return self._command(LedDmxProtocol.pattern(value), pattern_index=value)

    def set_music_mode(self, eq_mode: int) -> Future[None]:
        value = max(0, min(eq_mode, 255))
        return self._command(LedDmxProtocol.mic_eq(value), music_mode=value)

    def set_custom_pattern_mode(self, mode: CustomPatternMode) -> Future[None]:
        return self._command(LedDmxProtocol.custom_pattern_mode(mode), custom_pattern_mode=mode)

    def set_custom_pattern_direction(self, is_forward: bool) -> Future[None]:
        return self._command(LedDmxProtocol.custom_pattern_direction(is_forward), custom_pattern_forward=bool(is_forward))
    ...
    async def _write_and_update(self, packet: bytes, **changes: object) -> None:
        try:
    ...
    def _synchronize_transport_state(self) -> None:
        self._apply_transport_state(self._transport.current_state())
    ...
    def _submit(self, coroutine: Coroutine[Any, Any, None]) -> Future[None]:
        ...
        if self._loop is None or not self._loop.is_running():
            coroutine.close()
            future = Future()
            future.set_exception(RuntimeError("lighting controller event loop is unavailable"))
            return future
