...
class PcmMusicAnalysisSource:
    """Compose generic PCM capture with the shared music analyzer."""
    ...
    def start(self, callback: Callable[[MusicAnalysisState], None]) -> None:
        with self._lock:
            ...
            self._capture.start()
            self._running = True
            self._thread = threading.Thread(target=self._run, name="music-analysis-source", daemon=True)
            self._thread.start()
    ...
    def _run(self) -> None:
        try:
            while True:
                ...
                samples = np.asarray(frame.samples, dtype=np.float64).copy()
                with self._lock:
                    self._recent.append((frame.sample_rate_hz, samples))
