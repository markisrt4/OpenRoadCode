...
from .audio_analysis import AudioAnalyzer, AudioAnalysisState, SpectrumAnalysisMode
...
class MusicAnalyzer:
    ...
    _BANDS = {
        "kick": (42.0, 105.0), "tom_low": (70.0, 135.0), "tom_mid": (105.0, 195.0),
        "tom_high": (145.0, 280.0), "snare_body": (180.0, 420.0),
        "snare_crack": (1500.0, 5200.0), "cymbal": (6000.0, 15000.0),
    }
    ...
    @property
    def sensitivity(self) -> float: return self._sensitivity
    @property
    def calibrated(self) -> bool: return self._zero_samples is None and bool(self._noise_floor) and self._base.calibrated
    @property
    def spectrum_mode(self) -> SpectrumAnalysisMode: return self._base.spectrum_mode

    def set_sensitivity(self, value: float) -> None: self._sensitivity = max(0.25, min(2.0, float(value)))
    def set_spectrum_mode(self, mode: SpectrumAnalysisMode | str) -> None: self._base.set_spectrum_mode(mode)
    ...
    def _band_energies(self, frame: AudioFrame) -> dict[str, float]:
        ...
        if samples.size == 0: return {name: 0.0 for name in self._BANDS}
        windowed = samples * np.hanning(samples.size); spectrum = np.abs(np.fft.rfft(windowed)); freq = np.fft.rfftfreq(samples.size, d=1.0 / frame.sample_rate_hz); result: dict[str, float] = {}
        for name, (low, high) in self._BANDS.items():
            mask = (freq >= low) & (freq < high); result[name] = float(math.sqrt(np.mean(spectrum[mask] ** 2))) if np.any(mask) else 0.0
        return result

    def _classify(self, n: dict[str, float], rise: dict[str, float]) -> PercussionState:
        now = time.monotonic()
        candidates = {"kick": n["kick"]*.70+rise["kick"]*.95-n["tom_mid"]*.18,"tom_low":n["tom_low"]*.68+rise["tom_low"]*.85-n["kick"]*.24,"tom_mid":n["tom_mid"]*.72+rise["tom_mid"]*.90-n["kick"]*.18,"tom_high":n["tom_high"]*.72+rise["tom_high"]*.90-n["kick"]*.12,"snare":n["snare_body"]*.40+n["snare_crack"]*.65+rise["snare_crack"]*.85,"cymbal":n["cymbal"]*.62+rise["cymbal"]*1.05}
        thresholds={"kick":.20,"tom_low":.22,"tom_mid":.20,"tom_high":.22,"snare":.24,"cymbal":.24}; cooldown={"kick":.085,"tom_low":.115,"tom_mid":.105,"tom_high":.110,"snare":.155,"cymbal":.130}; active={name:0.0 for name in candidates}
        for name,score in sorted(candidates.items(),key=lambda item:item[1],reverse=True)[:2]:
            if score>=thresholds[name] and now-self._last_hit[name]>=cooldown[name]: active[name]=min(1.0,score); self._last_hit[name]=now
        return PercussionState(kick=active["kick"],snare=active["snare"],tom_low=active["tom_low"],tom_mid=active["tom_mid"],tom_high=active["tom_high"],cymbal=active["cymbal"])
