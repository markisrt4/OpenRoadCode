...
class PushedPcmMusicAnalysisSource:
    """Analyze PCM frames pushed by a browser, network client, or test harness.

    Unlike :class:`PcmMusicAnalysisSource`, this source does not own capture.
    The transport calls :meth:`push_frame`; all music interpretation remains in
    the shared Python ``MusicAnalyzer``.
    """

    def __init__(self, analyzer: MusicAnalyzer | None = None) -> None:
        self._analyzer = analyzer or MusicAnalyzer(spectrum_band_count=24)
        self._callback: Callable[[MusicAnalysisState], None] | None = None
        self._running = False
        self._recent: deque[tuple[int, np.ndarray]] = deque(maxlen=512)
        self._lock = threading.RLock()
        self._latest: MusicAnalysisState | None = None

    @property
    def sensitivity(self) -> float:
        return self._analyzer.sensitivity

    @property
    def calibrated(self) -> bool:
        return self._analyzer.calibrated
    ...
    def push_frame(self, samples: tuple[float, ...], sample_rate_hz: int) -> MusicAnalysisState:
        ...
        frame = AudioFrame(samples=samples, sample_rate_hz=sample_rate_hz)
        copied = np.asarray(samples, dtype=np.float64).copy()
        state = self._analyzer.analyze(frame)
        with self._lock:
            self._recent.append((sample_rate_hz, copied))
            self._latest = state
            callback = self._callback if self._running else None
    ...
    def recent_audio_pcm16(self, seconds: float = 6.0) -> bytes:
        ...
        if not frames:
            return b""
        sample_rate = frames[-1][0]
        wanted = max(1, int(sample_rate * seconds))
        samples = np.concatenate([samples for _, samples in frames])[-wanted:]
