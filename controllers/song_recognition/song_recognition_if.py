...
class SongRecognitionIf(ABC):
    ...
    def is_configured(self) -> bool:
        """Return whether required provider configuration is available."""
        raise NotImplementedError

    @property
    @abstractmethod
    def provider_name(self) -> str | None:
        """Return a human-readable provider name when configured."""
        raise NotImplementedError

    @abstractmethod
    def recognize(self, audio: bytes, *, sample_bytes: int | None = None) -> SongRecognitionResult | None:
        """Return the best matching song, or None when unmatched."""
        raise NotImplementedError
