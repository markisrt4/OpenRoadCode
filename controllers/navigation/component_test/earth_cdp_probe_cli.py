...
def _print_trace(controller: EarthCdpCameraController) -> None:
    ...
    for index, call in enumerate(calls, 1):
        ...
        for arg_index, arg in enumerate(args, 1):
            ...
            keys = arg.get("keys")
            if isinstance(keys, list) and keys: print(f"      keys: {', '.join(str(key) for key in keys)}")
...
def main() -> None:
    ...
    if args.my_location:
        if not controller.trigger_my_location(): raise SystemExit("Earth My Location command bridge is not available")
    ...
    if args.trace_commands:
        ...
        input(); _print_trace(controller); return
    ...
    print("Module keys:")
    for key in inspection.module_keys: print(f"  {key}")
