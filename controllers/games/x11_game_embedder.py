...
class X11GameEmbedder:
    """Use xdotool to reparent a child process window into an ORC host."""
    ...
    def embed(self, process_id: int, host_window_id: int, width: int, height: int) -> int:
        ...
        if not self.supported():
            raise RuntimeError("xdotool is required for embedded game windows")
    ...
    def clear(self) -> None:
        """Forget the embedded window after the game exits."""
