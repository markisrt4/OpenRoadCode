...
flowchart TD
    ...
    class encoder orcAdapter;
    class router,pipewire orcController;
    class audioIf orcMessage;
```
...
```mermaid
flowchart TD
    input["Input device"] --> audioIf["AudioControllerIf"] --> controller["PipewireAudioController"] --> wpctl["wpctl"] --> pipewire["PipeWire"]
    ...
    classDef orcAdapter fill:#fee2e2,stroke:#dc2626,color:#7f1d1d;
    classDef orcExternal fill:#f3f4f6,stroke:#6b7280,color:#1f2937;
    class input,wpctl,pipewire orcExternal;
    class audioIf orcMessage;
