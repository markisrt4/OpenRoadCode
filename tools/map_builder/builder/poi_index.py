...
def _transit_mode(tags: dict[str, str]) -> str | None:
    ...
    station = tags.get("station", "").casefold()

    def enabled(key: str) -> bool:
        return tags.get(key, "").casefold() in {"yes", "designated", "official"}

    # Prefer the more specific fixed-guideway modes before generic rail/bus tags.
    if enabled("subway") or station == "subway" or railway == "subway_entrance":
        return "subway"
    if enabled("tram") or railway == "tram_stop":
        return "tram"
    if (
        enabled("train")
        or enabled("light_rail")
        or enabled("monorail")
        or railway in {"station", "halt"}
    ):
        return "rail"
    if enabled("bus") or highway == "bus_stop":
        return "bus"
    return None


def _classification(tags: dict[str, str]) -> tuple[str, str, str, str | None] | None:
    ...
    if amenity in {"restaurant", "fast_food", "cafe", "food_court", "ice_cream"}:
        return "food", amenity, amenity, None
    if amenity in {"fuel", "charging_station"}:
        return "fuel", amenity, amenity, None
    if shop in {"supermarket", "grocery", "convenience"}:
        return "grocery", "shop", shop, None

    mode = _transit_mode(tags)
    if highway == "bus_stop":
        return "transit", "bus", "bus_stop", mode or "bus"
    if public_transport in {"platform", "station", "stop_position"}:
        return "transit", "public_transport", public_transport, mode
    if railway in {"station", "halt", "tram_stop", "subway_entrance"}:
        return "transit", "railway", railway, mode
    return None
...
def _decode_geojsonseq_record(line: str) -> dict:
    ...
    return json.loads(record) if record else {}
...
def _osm_id(tags: dict[str, str]) -> str | None:
    ...
    return f"osm:{tags.get('@type', tags.get('type', 'osm'))}:{osm_id}" if osm_id else None
...
def _insert_point_feature(connection: sqlite3.Connection, feature: dict) -> None:
    tags = _tags(feature)
    position = _representative_point(feature)
    ...
    if street_name and house_number:
        connection.execute(
            "INSERT OR REPLACE INTO address (id,house_number,street,unit,city,state,postcode,country,latitude,longitude) VALUES (?,?,?,?,?,?,?,?,?,?)",
            (
                object_id,
                house_number,
                street_name,
    ...
    if name and place_kind in {"city", "town", "village", "hamlet", "suburb", "neighbourhood", "quarter"}:
        connection.execute(
            "INSERT OR REPLACE INTO place (id,name,kind,state,country,latitude,longitude) VALUES (?,?,?,?,?,?,?)",
            (
                object_id,
                name,
                place_kind,
...
def _insert_street_feature(connection: sqlite3.Connection, feature: dict) -> None:
    ...
    object_id = _osm_id(tags)
    position = _representative_point(feature)
    if not name or not highway or object_id is None or position is None:
        return
    if highway in {"bus_stop", "crossing", "traffic_signals", "stop", "give_way", "street_lamp"}:
        return
    latitude, longitude = position
    connection.execute(
        "INSERT OR REPLACE INTO street (id,name,city,state,postcode,latitude,longitude) VALUES (?,?,?,?,?,?,?)",
        (
            object_id,
            name,
            tags.get("addr:city"),
